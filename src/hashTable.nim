import types, move, position, movegen
...
static:
  doAssert minHashSize * sizeof(HashTableEntry) <= 16_000_000
  # 16 MB
...
func getPv*(ht: var HashTable, position: Position): seq[Move] =
  ...
  while true:
    ...
    if entry.isEmpty or not (entry.bestMove in currentPosition.moves):
      return result
