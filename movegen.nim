...
func moves*(position: Position): seq[Move] =
  ...
  for target in position[position.us].singles and not position.occupancy:
...
func pieceDelta*(move: Move, position: Position): int =
  ...
  assert move.isLegal(position),
    "This function should be used on the position before the move"
